...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        mMap.addMarker(new MarkerOptions().position(place).title("place").icon(BitmapDescriptorFactory.fromResource(R.mipmap.boy)));
       mMap.addMarker(new MarkerOptions().position(me).title("me").icon(BitmapDescriptorFactory.fromResource(R.mipmap.position)));
        float zoomLevel = 15.0f; //This goes up to 21
       PolylineOptions rectOptions = new PolylineOptions()
               .add(new LatLng(lat, lon))
                .add(new LatLng(mylat, mylon));
